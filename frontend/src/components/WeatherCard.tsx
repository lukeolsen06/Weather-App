...
const WeatherCard = ({ weather }: WeatherProps ) => {
  ...
  const condition = weather.weather_descriptions[0].split(',')[0].trim()
  console.log(`Condition for ${weather.name}: is ${condition}`)
  const icon = weatherIconMap[condition.toLowerCase()] ?? <Cloud className="w-12 h-12"></Cloud>
    ...
    <div>
      <div className="p-4 flex items-center justify-center">
          <div className="flex flex-col bg-white rounded-2xl p-4 w-full max-w-xs shadow">
              ...
              <div className="text-sm text-gray-500">Last recorded: {weather.localtime}</div>
